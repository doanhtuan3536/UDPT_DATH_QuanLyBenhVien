...
import com.doanth.medical_service.dto.UserInfoDTO;
...
//        System.out.println(request);
    ...
    public UserInfoDTO getUserInfo(Integer doctorId, String accessToken) throws JwtValidationException {
...
//        String accessToken = login(serviceUsername, secretPassword);
        ...
        try{
            response= restTemplate.exchange(
                    url, HttpMethod.GET, request, UserInfoDTO.class
            );
            return response.getBody();
        }
        catch(HttpClientErrorException e){
    ...
    public List<UserInfoDTO> searchPatientInfo(Integer patientId, String patientFullName , String accessToken) throws JwtValidationException {
//        Claims claims = validateAccessToken(AccessTokenJwt);
//        return (DoctorInfoDTO) claims.get("doctorInfoDTO");
//        String accessToken = login(serviceUsername, secretPassword);
        String url =  null;
        if(patientId == null && patientFullName != null) url = userInfoUrl + "/patient?" + "fullName=" + patientFullName;
        else if(patientId != null && patientFullName == null) url = userInfoUrl + "/patient?" + "id=" + patientId;
        else if(patientId != null && patientFullName != null) url = userInfoUrl + "/patient?" + "id=" + patientId + "&fullName=" + patientFullName;
//        String url = userInfoUrl + "/patient?" + "id=" + patientId + "&fullName=" + patientFullName;
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + accessToken);
        HttpEntity<?> request = new HttpEntity<>(headers);
        ResponseEntity<List<UserInfoDTO>> response = null;
        try{
            response= restTemplate.exchange(
                    url, HttpMethod.GET, request, new ParameterizedTypeReference<List<UserInfoDTO>>() {}
            );
            return response.getBody();
        }
        catch(HttpClientErrorException e){
