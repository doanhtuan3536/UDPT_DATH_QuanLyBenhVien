...
public class MedicalServiceClient {
    ...
    private final String dischargeMedicalRecord = "http://localhost:8083/api/medical/records/discharge";

    public MedicalRecordShortDTO dischargeMedicalRecord(String accessToken, Integer medicalRecordId) throws RefreshTokenException, JwtValidationException {// URL của Auth Service

        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + accessToken);
        HttpEntity<MedicalRecordAddForm> request = new HttpEntity<>(headers);
//        System.out.println(request);
        String url = dischargeMedicalRecord + "/" + medicalRecordId;
        ResponseEntity<MedicalRecordShortDTO> response = null;
...
//                System.out.println("catch AppointmentServiceClient getSpecialties if Access token expired");
                ...
                try {
                    ...
                    if(error1.getErrors().contains("Prescription details not found")){
                        return null;
                    }
                }
