...
//        SecurityContextHolder.clearContext();
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public String handleAppointmentNotFoundException(HttpServletRequest request, Exception ex, Model model) {
//        ErrorDTO error = new ErrorDTO();
//
//        error.setTimestamp(new Date());
//        error.setStatus(HttpStatus.BAD_REQUEST.value());
...
//        LOGGER.error(ex.getMessage(), ex);
        ...
        model.addAttribute("errorMessage", "Lỗi hệ thống không tìm thấy lịch khám, vui lòng quay về trang home hoặc đăng nhập lại");
//        SecurityContextHolder.clearContext();
        return "error";
    }
